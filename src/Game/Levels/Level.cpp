...
Level::Level(const sf::Window *window,
             ...
             :  enemy_spawner(window->getSize().x/2.f - 180.f,
    ...
    // Enemies Killed
    this->enemies_killed.setFont(fonts["Langar"]);
    ...
    this->enemies_killed_count.setFont(fonts["Langar"]);
...
float Level::getCurrentHealthPercentage() const {
    return this->enemy_spawner.getCurrentHealthPercentage();
}
